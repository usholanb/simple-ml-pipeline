...
# class IndividualTF(BaseTorchModel):
...
#
#     train_dataset, val_dataset, test_dataset = baselineUtils.create_dataset_v1(args.dataset_folder, args.dataset_name, 0, args.obs,
#                                                    args.preds, delim=args.delim, train=False, eval=True,
#                                                    verbose=args.verbose)
#
#     model = individual_TF.IndividualTF(2, 3, 3, N=args.layers,
#                                        d_model=args.emb_size, d_ff=512, h=args.heads,
#                                        dropout=args.dropout, mean=[0, 0], std=[0, 0]).to(device)
...
#             inp = torch.cat((batch['src'][:, 0, 0:2].unsqueeze(1).to(device), inp), dim=1)
#             splitter = (-1 * torch.ones((inp.shape[0], 1, inp.shape[2]))).to(device)
#             nearest_neighbours = batch["nearest_neighbours"].float()[:, 0:args.obs, :, :2].to(device)
#             nearest_neighbours = nearest_neighbours.view(nearest_neighbours.shape[0],
#                                                          nearest_neighbours.shape[1] * nearest_neighbours.shape[2],
#                                                          nearest_neighbours.shape[3])
#             inp = torch.cat((inp, splitter, nearest_neighbours), dim=1)
...
#                                        ((batch['trg'][:, :, 0:2].to(device) -
#                                          mean.to(device))/std.to(device)).contiguous().view(-1, 2).to(device)).mean() + torch.mean(torch.abs(pred[:, :, 2]))
#             loss.backward()
#             optim.step()
